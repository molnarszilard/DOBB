...
def output_to_rotated_target_dpt(output, max_det=300):
    """Convert model output to target format [batch_id, class_id, x, y, w, h, conf] for plotting."""
    targets = []
    for i, o in enumerate(output):
        # if o.shape[1]==10:
        #     box, conf, cls, angle, Dpt,dpt_vis = o[:max_det].cpu().split((4, 1, 1, 1, 2, 1), 1)
        # elif o.shape[1]==8:
        #     box, conf, cls, angle, Dpt = o[:max_det].cpu().split((4, 1, 1, 1, 1), 1)
